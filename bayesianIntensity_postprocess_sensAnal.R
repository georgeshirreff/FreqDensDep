...
ward_nonlinear_sensAnal <- read_csv(paste0("output/ward_nonlinear_bayesIntensity_sensAnal.csv")) %>% 
...
# pop_bound_tab = ward_nonlinear_sensAnal %>% select(popcut, boundary) %>% unique

# pb = 3
# for(pb in 1:nrow(pop_bound_tab)){
#   this_popcut = pop_bound_tab$popcut[pb]
#   this_boundary = pop_bound_tab$boundary[pb]
#   
# }
# this_popcut = 0
tab = list()
...
#   filter(this_status == "PA", that_status == "PA", ward_id %in% c(2, 5, 8, 13))
  ...
  ward_aic = ward_rates_loglik %>% 
    ...
    #   filter(ward_id == 6) %>% 
    #   filter(this_status == this_stat, that_status == that_stat)
    tab[[paste0("popcut", this_popcut)]] <- ward_aic %>% 
      filter(ward_id != 15) %>% 
    ...
    tab[[paste0("popcut", this_popcut)]] %>%   
      write_tsv(paste0("AICtables/dAIC_intensity_", "popcut", this_popcut, ".tsv"))
      # write_tsv(paste0("AICtables/dAIC_intensity_popcut", this_popcut, "boundary", this_boundary, ".tsv"))
}
  

rbind(tab[[paste0("popcut", 0)]] %>% mutate(popcut = 0), 
      read_tsv("AICtables/dAIC_intensity.tsv") %>% mutate(popcut = 10), 
      # tab[[paste0("popcut", 10)]] %>% mutate(popcut = 10), 
tab[[paste0("popcut", 20)]] %>% mutate(popcut = 20)) %>% 
  select(Ward, popcut, everything()) %>% 
  arrange(Ward, popcut) %>% 
  write_tsv(paste0("AICtables/dAIC_intensity_", "popcuts.tsv"))
...
for(this_boundary in boundary_vec){
  ...
  #   filter(ward_id == 6) %>% 
  #   filter(this_status == this_stat, that_status == that_stat)
  tab[[paste0("boundary", this_boundary)]] <- ward_aic %>% 
    filter(ward_id != 15) %>% 
    filter(this_status == "ALL", that_status == "ALL") %>% 
    arrange(newID) %>% 
    transmute(Ward = newID, Constant = constant, Linear = linear, `Non-linear` = nonlin, `Constant diurnal` = cdiurnal, `Linear diurnal` = ldiurnal, `Non-linear diurnal` = ndiurnal) %>% 
  ...
  tab[[paste0("boundary", this_boundary)]] %>%   
    write_tsv(paste0("AICtables/dAIC_intensity_", "boundary", this_boundary, ".tsv"))
  # write_tsv(paste0("AICtables/dAIC_intensity_popcut", this_popcut, "boundary", this_boundary, ".tsv"))
}
  

rbind(tab[[paste0("boundary", 4)]] %>% mutate(boundary = 4), 
      tab[[paste0("boundary", 6)]] %>% mutate(boundary = 6), 
      # tab[[paste0("boundary", 8)]] %>% mutate(boundary = 8), 
      read_tsv("AICtables/dAIC_intensity.tsv") %>% mutate(boundary = 8),
      tab[[paste0("boundary", 10)]] %>% mutate(boundary = 10)) %>% 
  select(Ward, boundary, everything()) %>% 
  arrange(Ward, boundary) %>% 
  write_tsv(paste0("AICtables/dAIC_intensity_", "boundaries.tsv"))
