...
ward_intervals <- read_csv("FreqDens_ward_intervals.csv"
...
#### analysis table ####
popcut_vec = c(0, 10, 20)
ward_vec = c(0, ward_intervals$ward_id %>% unique)
stat_vec = c("ALL", "PA", "V", "PE")[1]
# stat_vec = "ALL"
per_vec = c("All", "Day", "Night")
boundary_vec = c(4, 6, 8, 10)
...
source("1.0_MCMC_multiTarget.R", local = T)
...
ward_nonlinear %>%  write_csv(paste0("analysis/ward_nonlinear_bayesIntensity_sensAnal", analysis_index, ".csv"))
